...
function initMarketTables() {
  ...
  // Reset header widths for Safari bug
	...
	// fix an issue where the datatables sometimes forget the right column
	var localEX = localStorage["exchangeSettings"];
	var settings = JSON.parse(localEX);
	showMarketColumn('BTC', settings.marketCols, false);
	showMarketColumn('ETH', settings.marketCols, false);
	showMarketColumn('XMR', settings.marketCols, false);
	showMarketColumn('USDT', settings.marketCols, false);
